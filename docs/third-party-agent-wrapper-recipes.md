...
| OpenCode | `python3 .codex/skills/room-skill/runtime/generic_agent_json_wrapper.py --agent-command 'python3 .codex/skills/room-skill/runtime/opencode_agent_wrapper.py --model opencode/gpt-5-nano'` | Uses the checked-in OpenCode wrapper, including narrow retry for transient local SQLite/WAL failures; validate with the host matrix before claiming live support. |
