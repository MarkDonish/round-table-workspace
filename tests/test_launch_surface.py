...
class LaunchSurfaceTest(unittest.TestCase):
    ...
    def test_docs_index_html_is_static_launch_landing_page(self) -> None:
        ...
        self.assertIn("Before and after using Round Table Workspace", text)
        self.assertIn("One confident answer moves toward merge.", text)
        self.assertIn("AI agent: ready to merge", text)
        self.assertIn("A small review panel names the next decision.", text)
        self.assertIn("ship-check: revise", text)
        self.assertIn("tighten evidence before merge", text)
        ...
        self.assertIn("grid-template-columns: repeat(4, 1fr)", text)
        self.assertIn("letter-spacing: 0", text)
    ...
    def test_competitive_insights_doc_is_original_and_source_attributed(self) -> None:
        ...
        self.assertIn("0xNyk/council-of-high-intelligence", text)
        self.assertIn("## Similarity Boundary: Council-Style Deliberation", text)
        self.assertIn("candidate reviewer generation", text)
        self.assertIn("signal-based reviewer selection", text)
        self.assertIn("bounded round-table debate", text)
