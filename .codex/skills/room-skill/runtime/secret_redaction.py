...
SENSITIVE_KEY_RE = re.compile(r"(?i)^(?:api[_-]?key|token|password|secret)$")
SENSITIVE_FLAG_RE = re.compile(r"(?i)^--(?:api[_-]?key|token|password|secret)$")

SECRET_PATTERNS = [
    re.compile(r"(?i)(authorization\s*:\s*)basic\s+[A-Za-z0-9._~+/=-]{8,}"),
    re.compile(r"(?i)(--(?:api[_-]?key|token|password|secret)(?:=|\s+))[^\s\"']{8,}"),
    re.compile(r"(?i)((?:\"(?:api[_-]?key|token|password|secret)\"|(?:api[_-]?key|token|password|secret))\s*:\s*\")[^\"]{8,}(\")"),
    ...
    if isinstance(value, list):
        ...
        for item in value:
            if redact_next:
                ...
                redact_next = False
                continue
            redacted_item = redact_sensitive_value(item)
